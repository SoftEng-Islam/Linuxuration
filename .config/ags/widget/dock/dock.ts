...
const Taskbar = (monitor) =>
	Widget.Box({
		...
		setup: (self) => {
			self.hook(Hyprland, (box, address) => box.attribute.add(box, address, self.monitor), "client-added").hook(
				Hyprland,
	...
	Widget.Box({
		...
		children: userOptions.dock.pinnedApps
			...
			.map(({ app, term = true }) => {
				const newButton = AppButton({
					// different icon, emm...
					icon: userOptions.dock.searchPinnedAppIcons ? searchIcons(app.name, icon_files) : app.icon_name,
					onClicked: () => {
						for (const client of Hyprland.clients) {
							if (client.class.toLowerCase().includes(term)) return focus(client);
						}
					...
					setup: (self) => {
						...
						self.hook(
							...
							(button) => {
								const running =
									Hyprland.clients.find((client) => client.class.toLowerCase().includes(term)) ||
									false;
...
export default (monitor = 0) => {
	const dockContent = Box({
		...
		children: [PinButton(), PinnedApps(), DockSeparator(), Taskbar(), LauncherButton()]
	});
	const dockRevealer = Revealer({
		attribute: {
			updateShow: (self) => {
				// I only use mouse to resize. I don't care about keyboard resize if that's a thing
				if (userOptions.dock.monitorExclusivity) self.revealChild = Hyprland.active.monitor.id === monitor;
